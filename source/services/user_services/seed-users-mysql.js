 ...
 * 
 * Requirements:
 * npm install mongoose uuid bcryptjs mysql2
 */
...
async function insertUsersToMySQL(mysqlPool, users) {
    const connection = await mysqlPool.getConnection();
    
    try {
        // Prepare batch insert query
        const insertQuery = `
            INSERT INTO user (id, email, firstname, lastname, created_at) 
            VALUES ?
        `;
        
        // Transform users data for MySQL
        ...
        await connection.query(insertQuery, [mysqlData]);
...
async function clearMySQLUsers(mysqlPool) {
    const connection = await mysqlPool.getConnection();
    
    try {
        await connection.query('DELETE FROM user');
        console.log('Cleared existing MySQL users');
    } finally {
        connection.release();
    }
}
...
async function seedUsers() {
    ...
    try {
        ...
        // Clear existing users from both databases
        await User.deleteMany({});
        console.log('Cleared existing MongoDB users');
        
        await clearMySQLUsers(mysqlPool);
        ...
        for (let i = 0; i < totalUsers; i += batchSize) {
            ...
            // Insert to both databases concurrently
            await Promise.all([
                User.insertMany(users, { ordered: false }),
                insertUsersToMySQL(mysqlPool, users)
            ]);
        ...
        // Display statistics
        const mongoStats = await User.aggregate([
            {
                $group: {
                    _id: '$role',
                    count: { $sum: 1 }
        ...
        const [mysqlCount] = await mysqlConnection.query('SELECT COUNT(*) as count FROM user');
        ...
        console.log('MongoDB user distribution by role:');
        mongoStats.forEach(stat => {
            console.log(`${stat._id}: ${stat.count} users`);
        });

        const totalMongoUsers = await User.countDocuments();
        console.log(`\nTotal MongoDB users created: ${totalMongoUsers}`);
        console.log(`Total MySQL users created: ${mysqlCount[0].count}`);
        ...
        if (sampleUsers.length > 0) {
            ...
            console.log('Sample MongoDB User:', {
                id: sample._id,
        ...
        // Sample MySQL user
        const mysqlSampleConnection = await mysqlPool.getConnection();
        const [mysqlSample] = await mysqlSampleConnection.query('SELECT * FROM user LIMIT 1');
        mysqlSampleConnection.release();
        
        if (mysqlSample.length > 0) {
            console.log('Sample MySQL User:', {
                id: mysqlSample[0].id,
                email: mysqlSample[0].email,
                firstName: mysqlSample[0].firstname,
                lastName: mysqlSample[0].lastname,
                createdAt: mysqlSample[0].created_at
            });
        }
        ...
        if (error.code === 'ECONNREFUSED') {
            console.error('Database connection refused. Please check if MongoDB and MySQL are running.');
        } else if (error.code === 'ER_ACCESS_DENIED_ERROR') {
            console.error('MySQL access denied. Please check your MySQL credentials.');
        } else if (error.code === 'ENOTFOUND') {
            console.error('Database host not found. Please check your database host configuration.');
        }
    } finally {
        // Close connections
        if (mongoConnection) {
